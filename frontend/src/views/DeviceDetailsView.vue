...
onMounted(() => {
  axios
    ...
    .then((res) => {
      ...
      if (device.value) {
        formUser.value = device.value.user;
        formOs.value = device.value.os;
        formSerial.value = device.value.serial;
      }
    })
    .catch((err) => {
  ...
  <v-row>
    <v-col cols="12" md="12">
      <UiParentCard>
        ...
        <v-card class="pa-6 mb-6" elevation="1" style="background-color: #f8f8f8; border-radius: 12px">
        ...
        <v-text-field v-model="formOs" label="Operating system and Raspberry Pi model" variant="outlined" density="compact" />
